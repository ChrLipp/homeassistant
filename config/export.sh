#!/bin/bash
################################################################################
# Home Assistant Export Script for new setup
# Version: 0.0.1
################################################################################
...
INPUT_DIR="./.storage"
BASE_OUTPUT_DIR="../export"

# Reverse timestamp (neueste oben sortiert)
TIMESTAMP="$(date '+%Y%m%d_%H%M%S')"
OUTPUT_DIR="$BASE_OUTPUT_DIR/$TIMESTAMP"

mkdir -p "$OUTPUT_DIR"

echo "📦 Export Home Assistant Storage → CSV"
echo "📁 Output-Verzeichnis: $OUTPUT_DIR"

#############################
# 1. Integrationen
#############################
echo "→ integrations.csv"

jq -r '
  ["domain","title","source","state","data","options"],
  (
    .data.entries[] |
    [
      .domain,
  ...
  | @csv
' "$INPUT_DIR/core.config_entries" \
> "$OUTPUT_DIR/integrations.csv"
...
jq -r '
  ["entity_id","platform","device_id","area_id","original_name","disabled","capabilities"],
  (
    .data.entities[] |
    [
      .entity_id,
      ...
      (.area_id // ""),
      (.original_name // ""),
      (if .disabled_by then "yes" else "no" end),
      (.capabilities | tostring)
    ]
  )
  | @csv
' "$INPUT_DIR/core.entity_registry" \
...
jq -r '
  ["device_id","name","manufacturer","model","area_id","via_device_id"],
  (
    .data.devices[] |
    [
      .id,
  ...
  | @csv
' "$INPUT_DIR/core.device_registry" \
> "$OUTPUT_DIR/devices.csv"

echo "✅ Fertig! CSVs liegen in: $OUTPUT_DIR"
...
TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI4NzU5ZWIyZTY3Zjk0OGEyYTQ1ZDkzZDczMWZkYTVmMyIsImlhdCI6MTc2NzE4NDI3OCwiZXhwIjoyMDgyNTQ0Mjc4fQ.LDgxC4BTtTuyzdgCUUbCqjX7D3NCGAkPvxWfp07RZPg"

# Basis-Verzeichnisse
HA_CONFIG_DIR="/Volumes/docker/homeassistant/config"
